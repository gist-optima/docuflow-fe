...
import { snippets } from "src/dummy/dummy";
import { DetailedProject, PullRequest, QueryKey } from "src/types/types";
...
const ShowDiff = ({ pullRequest, project, containerId }: ShowDiffProps) => {
  const currentBranchVersions = project.Version.filter(
    (version) => version.tag === pullRequest.toTag,
  );
  ...
  const diffBranchVersions = project.Version.filter(
    (version) => version.tag === pullRequest.fromTag,
  );
  ...
  const { data } = useQuery({
    queryKey: [
      QueryKey.GetBranchDiff,
      {
        projectId: project.id,
        versionId: newestCurrentBranchVersion.id,
        diffVersionId: newestDiffBranchVersion.id,
        containerId: containerId,
      },
    ],
    queryFn: checkBranchDiff,
  });
